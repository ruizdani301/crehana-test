import pytest
from application.schemas import TaskCreate, TaskStatus, TaskPriority, TaskOut, UserCreate, Token
...
@pytest.mark.parametrize("invalid_priority, expected_msg", [
...
def test_task_out_invalid_priority(invalid_priority, expected_msg):
    with pytest.raises(ValidationError) as exc_info:
        TaskOut(priority=invalid_priority, status=TaskStatus.pending, title="title", description="description", id=1, list_id=1)
